...
/// Finder Sync Extension adding "Send with RustDrop" to the Finder context menu.
///
...
class FinderSync: FISyncExtension {
    ...
    override init() {
        ...
        NSLog("RustDrop Finder Extension: Initialized")
    }
    ...
    override func menu(for menuKind: FIMenuKind) -> NSMenu {
        ...
        let sendItem = NSMenuItem(
            title: "Send with RustDrop",
            action: #selector(sendWithRustDrop(_:)),
            keyEquivalent: ""
        )
    ...
    @objc func sendWithRustDrop(_ sender: Any?) {
        guard let target = FIFinderSyncController.default().targetedURL(),
              let items = FIFinderSyncController.default().selectedItemURLs(),
              !items.isEmpty
        else {
            NSLog("RustDrop Finder: No files selected")
            return
        }

        let filePaths = items.map { $0.path }
        NSLog("RustDrop Finder: Sending \(filePaths.count) files")
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 2.0) {
            ...
            if peers.isEmpty {
                ...
                alert.informativeText = "Make sure the receiving device is on the same network and has RustDrop running."
        ...
        if alert.runModal() == .alertFirstButtonReturn {
            ...
            if idx >= 0 && idx < peers.count,
               let peerId = peers[idx]["id"] as? String {
                _ = RustBridge.shared.sendFiles(peerId: peerId, filePaths: filePaths)
            }
        }
    }
}
