...
use rustdrop_core::config::Config;
use rustdrop_core::types::DeviceInfo;
use rustdrop_discovery::{PeerEvent, ServiceBrowser, ServiceRegistrar};
use rustdrop_network::TransferListener;
use rustdrop_protocol::{
    IncomingTransferDecision, RecvTransfer, SendTransfer, TransferEvent,
};
use rustdrop_security::IdentityKeys;
...
/// Opaque handle to the RustDrop engine.
pub struct RustDropEngine {
    runtime: Runtime,
    config: Config,
...
static ENGINE: Mutex<Option<Box<RustDropEngine>>> = Mutex::new(None);
...
/// Initialize the RustDrop engine.
///
/// # Safety
/// `config_path` must be a valid null-terminated C string or NULL for defaults.
#[no_mangle]
pub unsafe extern "C" fn rustdrop_init(config_path: *const c_char) -> c_int {
    // Initialize tracing
    let _ = tracing_subscriber::fmt()
        .with_env_filter("rustdrop=info")
        .try_init();
    ...
    *ENGINE.lock() = Some(Box::new(engine));
    info!("RustDrop engine initialized");
    0
}

/// Start mDNS discovery (register + browse).
#[no_mangle]
pub extern "C" fn rustdrop_start_discovery() -> c_int {
...
///
/// # Safety
/// Caller must free the returned string with `rustdrop_free_string`.
#[no_mangle]
pub extern "C" fn rustdrop_get_peers() -> *mut c_char {
...
#[no_mangle]
pub unsafe extern "C" fn rustdrop_send_files(
    peer_id: *const c_char,
...
/// Free a string allocated by RustDrop.
///
/// # Safety
/// `ptr` must have been returned by a `rustdrop_*` function.
#[no_mangle]
pub unsafe extern "C" fn rustdrop_free_string(ptr: *mut c_char) {
    if !ptr.is_null() {
        drop(CString::from_raw(ptr));
    }
}

/// Shut down the RustDrop engine.
#[no_mangle]
pub extern "C" fn rustdrop_shutdown() -> c_int {
    let mut guard = ENGINE.lock();
    if let Some(engine) = guard.take() {
        drop(engine);
        info!("RustDrop engine shut down");
    }
    0
}
