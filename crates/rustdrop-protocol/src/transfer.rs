...
use rustdrop_core::types::*;
use rustdrop_network::{connect, SecureTransport, TransferListener};
use rustdrop_security::{IdentityKeys, NoiseSession};
use rustdrop_storage::{
...
impl RecvTransfer {
    ...
    async fn save_resume_state(
        ...
        let files: Vec<_> = writers
            ...
            .filter_map(|(i, w)| {
                w.as_ref().map(|writer| {
                    rustdrop_storage::resume::FileResumeState {
                        file_index: i as u32,
