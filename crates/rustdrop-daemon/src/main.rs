//! RustDrop Daemon — background service for file transfer.
//!
...
use rustdrop_core::types::format_bytes;
...
#[derive(Parser)]
#[command(name = "rustdrop")]
#[command(about = "RustDrop — Cross-platform local file transfer")]
#[command(version)]
struct Cli {
    ...
    tracing_subscriber::fmt()
        .with_env_filter(
            tracing_subscriber::EnvFilter::try_from_default_env()
                .unwrap_or_else(|_| format!("rustdrop={log_level}").parse().unwrap()),
        )
        .with_target(false)
