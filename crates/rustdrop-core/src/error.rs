//! Error types for the RustDrop system.
//!
//! Each subsystem has its own error variant, all unified under [`RustDropError`].

use std::path::PathBuf;
use thiserror::Error;
use uuid::Uuid;

/// Top-level error type encompassing all RustDrop subsystems.
#[derive(Debug, Error)]
...
mod tests {
    ...
    fn error_conversion() {
        ...
        let _top: RustDropError = sec_err.into();

        let net_err = NetworkError::ConnectionReset;
        let _top: RustDropError = net_err.into();
    }
}
