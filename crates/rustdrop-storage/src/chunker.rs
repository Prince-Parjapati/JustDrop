...
use bytes::Bytes;
use rustdrop_core::error::StorageError;
use rustdrop_core::types::{ChunkData, ChunkId, Sha256Hash};
...
mod tests {
    ...
    async fn chunk_small_file() {
        let tmp = std::env::temp_dir().join("rustdrop_chunker_test");
    ...
    async fn chunk_with_remainder() {
        let tmp = std::env::temp_dir().join("rustdrop_chunker_test2");
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).unwrap();
    ...
    async fn hash_file_matches_manual() {
        let tmp = std::env::temp_dir().join("rustdrop_hash_test");
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).unwrap();

        let file_path = tmp.join("test.bin");
        let data = b"hello rustdrop";
        std::fs::write(&file_path, data).unwrap();
    ...
    async fn stream_chunks_through_channel() {
        let tmp = std::env::temp_dir().join("rustdrop_stream_test");
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).unwrap();
