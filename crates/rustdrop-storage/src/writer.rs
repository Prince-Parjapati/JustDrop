...
use rustdrop_core::types::{ChunkId, Sha256Hash, TransferId};
...
impl ChunkWriter {
    ...
    pub async fn new(
        ...
        let temp_path = dest_path.with_extension("rustdrop_partial");
...
mod tests {
    ...
    async fn write_and_finalize() {
        let tmp = std::env::temp_dir().join("rustdrop_writer_test");
    ...
    async fn missing_chunks_tracking() {
        let tmp = std::env::temp_dir().join("rustdrop_writer_missing_test");
        let _ = std::fs::remove_dir_all(&tmp);
        std::fs::create_dir_all(&tmp).unwrap();
